...
Node.prototype.ons = function(configs, psel, data) {
  ...
  for (name in configs) {
    ...
    for (event in obj) {
      (function(event, eventFun) {
        node.on(event, psel + name, data, function(){
          var args = Array.prototype.slice.call(arguments, 0);
          args.push(data);
          eventFun.apply(this, args);
        });
      }(event, obj[event]));
    }
  }
...
Node.prototype.offs = function(configs, psel) {
  psel = psel ? (psel + ' ') : '';
...
function runFunction(mainId, dataObj, scope, isForAsyn) {
  ...
  var funstr = 'var global = this;\n'
             + declareMainVar(names, true).join('\n')
