...
Valley.getRand = function(num) {
  var num = num || 10000;
  return Math.floor(Math.random() * num);
};

Valley.queue = function(list, data, scope){
  // console.log(list, data, scope);
...
Valley.showPage = function(path, params) {
  ...
  var page = Valley(path).run();
  page.getPromise().then(function(con){
...
function runFunction(mainId, dataObj, scope, isForAsyn) {
  ...
  Valley.debug && console.log(names, funstr);
...
var Controller = Class.declare({
  ...
  init: function(data) {
    ...
    this.data.el = this.data.el || this.el || Valley.dataObj.element;
    // this.tpl = this.tpl || this.id;
    this._bind();
  },

  renderData: function(data, tpl, el, isAppend) {
    var con = this;
    var dataJob = this.prepareData(data);
  ...
  render: function(data, tpl, el, isAppend) {
    return this.renderData(data, tpl, el, isAppend);
  },

  prepareEl: function(el) {
    var el = el || this.data.el || this.parent.data.el || Valley._config.el;
  ...
  prepareTpl: function(tpl) {
    ...
    return this.data.tpl && Valley.initTplObj(this.data.tpl).then(function(res){
      // console.log(res);
      return res;
    }) || Promise.resolve('');
  },
  ...
  _bind: function() {
    ...
    this.prepareEl().then(function(element){
      ...
      element.ons(con.elEventObj, '', data);
      con.bind && con.bind();
    });
  }
});
