...
Valley.init = function(config, callback) {
  ...
  this._prepares();
  require(this._config.prepareList, function(){
...
EventTarget.prototype.one = function(types, selector, data, fn) {
  this.on(types, selector, data, fn, 1);
};

EventTarget.prototype.ons = function(configs, psel) {
  var name, value, obj;
  var psel = psel ? (psel + ' ') : '';
  var event;
  for (name in configs) {
    ...
    for (event in obj) {
      this.on(event, psel + name, obj[event]);
    }
  }
};
...
NodeList.prototype.forEach = function(callback, thisArg) {
  ...
  return arr.forEach(callback, thisArg || global);
}
...
EventTarget.prototype.on = function(types, selector, data, fn, one) {
  // by jquery
  $(this).on(types, selector, data, fn, one);
};

EventTarget.prototype.off = function(types, selector, fn) {
  $(this).off(types, selector, fn);
};

EventTarget.prototype.trigger = function(event, data, elem, onlyHandlers) {
...
function insertTag(src) {
  ...
  if (res && res[1]) {
    switch(res[1]) {
    case 'css':
      return insertCss(res[1])
    case 'js':
    default:
      return insertJs(res[1]);
    }
  }
...
Valley.setOrigin = function() {
  this._config.apiConfig.origin = [
    this._config.apiConfig.protocol,
    '://',
    this._config.apiConfig.host,
    ':',
    this._config.apiConfig.port].join('');
    ...
    if (tagInfo) {
      ...
      case 'include':
        ...
        itpl = 'var vtmpFunc = function(vtmpInput){'
             ...
             + '};'
             + 'vtmpArr.push(vtmpFunc.call(this, Valley.extend(vtmpInput, {' + iarr.join(',') + '})));';
        tags.push(itpl);
        break;
      case 'if':
      case 'for':
...
Valley.extend(Controller.prototype, {
  ...
  init: function() {
    ...
    this._data = Valley.extend(Valley.dataObj, this.data);
  ...
  getViewId: function() {
    return this.data.tplId || this.data.pageId || this.data.defaultPageId;
  },
  ...
  renderSimplePage: function(tplName) {
    return Valley.getTplContent(tplName || this.getViewId());
  },
  renderTpl: function(data, tplName) {
    var tplName = tplName || this.getViewId();
    var data = data || {};
    return Valley.tpl(tplName, data, this._data);
  },
