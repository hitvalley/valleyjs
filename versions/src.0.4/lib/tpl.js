...
// var Valley = Valley.Valley || Valley;
Valley.tplObj = {};
...
Valley.asynFunc = function(asynConfig, data, callback) {
  var i,
      names = [],
      promise,
      jobs = [];
...
Valley.asynBlockFunc = function(asynObj, data) {
  var key,
      asynArr,
      names = [],
      promise,
  ...
  for (key in asynObj) {
    ...
    fetch(key).then(function(res){
      ...
      asynArr.forEach(function(asyn){
        ...
        Valley.replaceChildWithHTML(tmp, html);
      });
    });
  }
};
...
Valley.sprintf = sprintf;var rInclude = /\{include\s+([^{}]+)\}/g;
...
Valley.tpl = function(mainTid, data, scope, asynConfig) {
  var scope = scope || Valley;
  var signId = 'tpl_' + Date.now();
  var asynObj = {};
  data.__asynData = {};
  Valley.asynFunc(asynConfig, data);
  var funcStr = tplToFuncStr(mainTid, asynObj, signId);
  Valley.asynBlockFunc(asynObj, data);
  var html = runTplAsFunction(funcStr, data, scope);
  return html;
};

Valley.vtpl = function(tpl, data, scope) {
  var scope = scope || Valley;
...
Valley.getFunc = function(mainTid, scope) {
  var scope = scope || Valley;
  var funcStr = tplToFuncStr(mainTid, Valley.tplObj);
